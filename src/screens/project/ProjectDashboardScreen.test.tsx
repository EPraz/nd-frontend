...
describe("ProjectDashboardScreen", () => {
  ...
  it("GIVEN the dashboard opens WHEN the screen renders SHOULD default to the current layout", () => {
    render(<ProjectDashboardScreen />);

    expect(screen.getByText("ARXIS Dashboard")).toBeOnTheScreen();
    expect(screen.getByText("centerMid")).toBeOnTheScreen();
    expect(screen.queryByText("Temporary comparison")).toBeNull();
  });

  it("GIVEN review mode is selected WHEN the toggle is pressed SHOULD show the curated comparison layout", () => {
    render(<ProjectDashboardScreen />);

    fireEvent.press(screen.getByText("Review layout"));
